...
func NewProvider(kubeClient kubernetes.Interface, kubeController k8s.Controller, stop chan struct{}, providerIdent string, cfg configurator.Configurator) (endpoint.Provider, error) {
	...
	informerCollection.Endpoints.AddEventHandler(k8s.GetKubernetesEventHandlersDEPRECATED("Endpoints", "Kubernetes", client.announcements, shouldObserve))
	informerCollection.Pods.AddEventHandler(k8s.GetKubernetesEventHandlersDEPRECATED("Pods", "Kubernetes", client.announcements, shouldObserve))
